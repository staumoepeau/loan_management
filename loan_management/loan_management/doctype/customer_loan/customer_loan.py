...
#	def on_submit(self):
	...
	def make_jv_entry(self):
		...
		journal_entry.user_remark = _('Against Customer Loan: {0}').format(self.name)
...
#			from `tabGL Entry` where against_voucher_type="Customer Loan" and against_voucher = %s""", 
#			(self.name), as_dict=1)[0]
#		if disbursement.disbursed_amount == self.loan_amount:
#			frappe.db.set_value("Customer Loan", self.name , "status", "Fully Disbursed")
#			
#			frappe.db.sql("""Update `tabCustomer Loan` set status="Fully Disbursed" where name = %s """, (self.name))
...
#			frappe.db.set_value("Customer Loan", self.name , "status", "Partially Disbursed")
#			frappe.throw(_("Partially Disbursed"))
#		if disbursement.disbursed_amount == 0:
#			frappe.throw(_("Sanctioned"))
#			frappe.db.set_value("Customer Loan", self.name , "status", "Sanctioned")
#		if disbursement.disbursed_amount > self.loan_amount:
#			frappe.throw(_("Disbursed Amount cannot be greater than Loan Amount {0}").format(self.loan_amount))
#		if disbursement.disbursed_amount > 0:
#			frappe.db.sql("""Update `tabCustomer Loan` set disbursement_date="Partially Disbursed" where name=%s """, self.name)
#			frappe.db.set_value("Customer Loan", self.name , "disbursement_date", disbursement.posting_date)	
...
@frappe.whitelist()
def get_customer_loan_application(customer_loan_application):
	customer_loan = frappe.get_doc("Customer Loan Application", customer_loan_application)
	if customer_loan:
		return customer_loan.as_dict()

@frappe.whitelist()
def make_jv_entry(customer_loan, company, customer_loan_account, customer, loan_amount, payment_account):
	journal_entry = frappe.new_doc('Journal Entry')
	journal_entry.voucher_type = 'Bank Entry'
	journal_entry.user_remark = _('Against Customer Loan: {0}').format(customer_loan)
